...
5. 将一条留言的 `is_approved` 勾选为 `true`，该留言就会显示在网站首页；取消勾选即可下架。

留言表没有向匿名访客开放直接读取、修改或删除权限，网页只能调用受限函数新增留言，并通过另一个受限函数读取已审核内容。前端会限制留言为 2–500 个字符，并对同一浏览器设置一分钟提交间隔；若网站公开访问量较大，建议后续再增加验证码或 Edge Function 服务端限流。
